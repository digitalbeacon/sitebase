	...
	if ($.fn.modalBox) {
		...
		$.digitalbeacon.modalBox = function(options) {
			...
			if (options.ajax || options.ajaxPost) {
				...
				let modalBox = $.digitalbeacon.getModalBox();
				if (modalBox.length) {
					...
					let modalBoxContent = modalBox.find('.modalBoxBodyContent');
					let ul = modalBoxContent.find('> ul#modalBoxStack');
					let $currentContents = modalBoxContent.find('> :not(ul#modalBoxStack)');
					let updateElement = modalBoxContent;
					let elementsToHide;
					if (replaceLi) {
					...
					let content = $.digitalbeacon.modalBox.defaults.noDataText;
					if (options.content) {
					...
					//modalBox.fadeTo(0, 1);
					modalBox.data('modalBoxOptions', options);
					let faderLayer = $('#modalBoxFaderLayer');
		...
		$.digitalbeacon.modalBoxOnClose = function(handler) {
			setTimeout(function() {
				let modalBox = $.digitalbeacon.getModalBox();
				if (modalBox.length) {
					let ul = modalBox.find('.modalBoxBodyContent > ul#modalBoxStack');
					let container = modalBox;
					if (ul.length) {
						container = ul.find('> li:last');
					}
					let options = container.data('modalBoxOptions');
					if (!options.close) {
						options.close = handler;
					}
					else {
						let proxiedHandler = options.close;
		...
		$.digitalbeacon._updateModalBoxContent = function(modalBox, elementToUpdate, elementsToHide, content, options) {
			let loading = modalBox.hasClass('loading');
			let reposition = options.reposition || loading;
			if (reposition) {
		...
		$.digitalbeacon._executeCloseHandler = function(modalBox, removeItem, recenter) {
			...
			if (modalBox.length) {
				let modalBoxContent = modalBox.find('.modalBoxBodyContent');
				let lastItem = modalBoxContent.find('> :not(ul#modalBoxStack)');
				let ul = modalBoxContent.find('> ul#modalBoxStack');
				let options;
				if (ul.length) {
					let li = ul.find('> li:last');
		...
		-----------------------------------------------------------------------------*/
		let proxiedClose = $.fn.modalBox.close;
